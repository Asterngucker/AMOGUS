...
class Rigid_body:
    ...
    def __init__ (body, part_set, vx=0, vy=0, omeg=0):
        ...
        for part in part_set:
            ...
            m_y += part.y*part.m
            m_tot += part.m
            j += (part.x**2 + part.y**2 + 0.4*part.r**2)*part.m #учитывается момент инерции детали-сферы.
    ...
    def calc_coll(part_1, part_2):
        ...
        Импульс передаются через линию соединяющую центры шаров-деталей.
        '''
        body_1 = part_1.body
        body_2 = part_2.body
        if body_1 is not body_2 and (part_1.x - part_2.x)**2 + (part_1.y - part_2.y)**2 < (part_1.r + part_2.r)**2:
    ...
    def calc_coll_massive(part, part_massive):
        '''
        Столкновение 2 тел, причем 2-е считается бесконечно массивным.
        '''
        part_1 = part
        part_2 = part_massive
        body_1 = part_1.body
    ...
    class Motion_derivative:  #класс для хранения производных от параметров системы твёрдых тел в методе Рунге - Кутты
        def __init__ (k):
            k.x    = dict()  #(!) названия переменных обозначают производные от данных величин (!)
            k.y    = dict()
            k.alph = dict()
            k.vx   = dict()
            k.vy   = dict()
    ...
    def get_grav_derivative(massive_bodies, all_bodies): #вычисляет производные параметров твёрдых тел при гравитационном взаимодействии
        G = 6.67E-11
                ...
                if grav_body is not body:
                    for part in body.part:
                        for grav_part in grav_body.part:
                            ...
                            f_div_r = G*grav_part.m*part.m/((delta_x)**2 + (delta_y)**2)**1.5     #рассчёт силы взаимодействия 2 сфер по осям.
                            f_x = f_div_r * delta_x
                            f_y = f_div_r * delta_y
                            
                            k.vx[body] += f_x
                            k.vy[body] += f_y
                            k.omeg[body] += ((part.x - body.x) * f_y - (part.y - body.y) * f_x)   #момент сил, действующий на всё тело
            k.vx[body]   /= body.m #получаем производные скорости и угловой скорости
            k.vy[body]   /= body.m
            k.omeg[body] /= body.j
        return k
    ...
    def runge_kutta_n_body (massive_bodies, all_bodies, time_step):
        ...
        for body in all_bodies:            
            ...
            body.apply_derivative(k2, h/2)  #снова перемещаем, для третьего шага
            body.refresh_redundant()
